...
export async function getInstallStatus(options = {}) {
  ...
  const configuredBinDir = typeof config?.binDir === "string" && config.binDir ? config.binDir : undefined;
  const binDir = options.binDir || configuredBinDir || getDefaultBinDir();
...
export async function uninstallFlowTidy(options = {}) {
  ...
  return { removed, preserved, configFile: CONFIG_FILE, binDir };
}
